...
外貨取引のある企業では、経理処理や売上レポートのために「その日の為替レート」を毎日参照する業務が発生します。
担当者が毎朝レートを調べて転記する運用を想定し、それを
**取得 → 検証 → 蓄積 → 集計 → グラフ更新まで全自動化する日次バッチ**
として設計・実装しました。

システム開発全般で生きる **DB設計とデータ処理** を実務レベルで実践することを目的に、
要件定義・設計・実装・運用設計（自動実行・再実行時の冪等性）まで一人で担当しています。

## このシステムでできること（想定ユースケース）

- 毎朝9時に前営業日の為替レートを自動取得し、MySQLに蓄積
    ⇒手作業の転記が不要に

- 欠損・異常値は品質チェックで自動除外し、除外内容を記録
    ⇒誤ったレートで経理処理する事故を防止

- 前日比・7日移動平均を自動集計し、CSVとグラフで出力
    ⇒レポート作成の下準備が完了した状態に

- 障害等で再実行しても重複登録されない冪等設計
    ⇒運用担当者が安心してリトライできる

## 作成者について

- MySQL・SQLの指導経験1年半（未経験者向けJava/DB研修講師）
- Oracle Master Silver SQL 保有／SQL Server 実務使用経験あり

## 本プロジェクトのAI利用方針

- 設計・実装は自分で行い、AIはレビューと骨格提示のみに限定する「コメント駆動ワークフロー」で開発
